...
function printCars(data) {
  ...
  if (data.error) elemContent.innerHTML = data.error;
    ...
    for (let i = 0; i < data.result.length; i++) {
      ...
      newSummary.innerText = `BRAND: ${car.manufacturer.name}`;
      newDetails.appendChild(newSummary);
      for (const property in car.manufacturer) {
        if (!['name', '_id', '__v'].includes(property)) {
          const newP = document.createElement('p');
          newP.innerText = `${property}: ${car.manufacturer[property]}`.toUpperCase();
